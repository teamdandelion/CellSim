...
def manage_resource_flow(cell):
    ...
    #   min_sugar
    #   growth_demand
    min_sugar = cell.sugar_consumption * 5
    free_sugar = cell.sugar - min_sugar
    
    min_water = cell.water_consumption * 5
    free_water = cell.water - min_water
    
    adj_sugar_demand, adj_water_demand = get_adjacent_demand(cell)
...
def distribute(cell, amount, resource, pattern):
    ...
    if pattern in ('high', 'low'):
        adj_demand = get_adjacent_demand(cell)
        if resource == 'sugar':
        ...
        if adj_demand == 0:
            ...
            distribution_factor = amount / adj_demand
        # The proportion of amount demanded to give to each adjacent cell
        
        for dir, adjcell in cell.adjacent.iteritems():
            if adjcell != 'EMPTY':
                amt_to_send = distribution_factor * getCellDemand(adjcell, resource, pattern)
                xferResource(cell, dir, amt_to_send, resource)
    else:
        if pattern != 'even':
            debug()
        num_adjacent_cells = 8 - cell.free_spaces 
        if num_adjacent_cells == 0:
            amt_to_send = 0
        else:
            amt_to_send = amount / num_adjacent_cells
        for dir, adjcell in cell.adjacent.iteritems():
            if adjcell != 'EMPTY':
                xferResource(cell, dir, amt_to_send, resource)
...
#def dna_stem(cell):
    ...
    # Idea: use 
    # 
def getCellDemand(cell, resource, priority):
    demand = getMem(cell, 'demand')
    if demand == 0:
        return 0
    else:
        if resource == 'sugar':
